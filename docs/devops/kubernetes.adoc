...
**** #"*Kubernetes*" image:images/kubernetes.svg[width=40] wants to abstract away the *"container runtime"* or container technologies (like *docker*) so that you can replace them **(containers). **If you want to avoid vendor lock-in.#
...
These practices are related to the operationss, that's the reason why
*#DevOps#*--to-> *#DevSecOps#*
